...
c    (arXiv:0711.0159). Submitted to Phys. Rev. C.
      ...
      real*8 f1tmp,f2tmp,w1tmp,w2tmp
      integer ism,i
       real*8 fyd(20)/ 0.4965, 0.4988, 0.4958, 0.5008, 0.5027, 0.5041,
     >  0.5029, 0.5034, 0.4993, 0.5147, 0.5140, 0.4975, 0.5007, 0.4992,
     >  0.4994, 0.4977, 0.5023, 0.4964, 0.4966, 0.4767/
...
! Need avp2f term to get right behavior x>1!
     ...
     >    qv**2 + 2. * qv * pz - avp2f(ism)
      ...
      if(W2.gt.6.0) then
         call ineft(q2,sqrt(w2),w1tmp,w2tmp,dble(2.0))
         F1tmp=am*W1tmp
         F2tmp=nu*W2tmp
         if(W2.le.10.0) then
            F1=(10.0-W2)/4.0*F1 + (W2-6.0)/4.0* F1tmp
c           F2=(10.0-W2)/4.0*F2 + (W2-6.0)/4.0* F2tmp
c            R=(F2/F1)*(mp/nu)*(1.0+nu**2/q2)
c            sigT = F1/((w2-mp2)/8./pi/pi/alpha/0.3894e3)
c            sigL = R*sigT
         else
            F1=F1tmp
c            F2=F2tmp
...
c            sigT = F1/((w2-mp2)/8./pi/pi/alpha/0.3894e3)
c            sigL = R*sigT
c            write(6,*) 'cheesy poofs', Q2,w2,w1tmp,w2tmp,F1,F2,R
         endif
      endif
      ...
      if(Q2.gt.8.0) then
         ...
         F2tmp=nu*W2tmp
         if(Q2.le.10.0) then
            F1=(10.0-W2)/2.0*F1 + (W2-8.0)/2.0* F1tmp
c            F2=(10.0-W2)/2.0*F2 + (W2-8.0)/2.0* F2tmp
c            R=(F2/F1)*(mp/nu)*(1.0+nu**2/q2)
c            sigT = F1/((w2-mp2)/8./pi/pi/alpha/0.3894e3)
c            sigL = R*sigT
         else
...
c    DG: add call to ineft for large W, Q2
     ...
     & 0.18951E+01,0.51376E+00,0.00000E+00,0.42802E+01,0.00000E+00 /
      ...
      R = sigL/sigT
      F2 = (nu/mp)*F1*(1.0+R)/(1.0+nu**2/q2)

      if(W2.gt.6.0) then
         call ineft(q2,sqrt(w2),w1tmp,w2tmp,dble(1.0))
         F1tmp=mp*W1tmp
         F2tmp=nu*W2tmp
         if(W2.le.10.0) then
            F1=(10.0-W2)/4.0*F1 + (W2-6.0)/4.0* F1tmp
            F2=(10.0-W2)/4.0*F2 + (W2-6.0)/4.0* F2tmp
            R=(F2/F1)*(mp/nu)*(1.0+nu**2/q2)
            sigT = F1/((w2-mp2)/8./pi/pi/alpha/0.3894e3)
            sigL = R*sigT
         else
            F1=F1tmp
            F2=F2tmp
...
c            write(6,*) 'cheesy poofs', Q2,w2,w1tmp,w2tmp,F1,F2,R
      ...
      if(Q2.gt.8.0) then
         ...
         if(Q2.le.10.0) then
            F1=(10.0-W2)/2.0*F1 + (W2-8.0)/2.0* F1tmp
            F2=(10.0-W2)/2.0*F2 + (W2-8.0)/2.0* F2tmp
            R=(F2/F1)*(mp/nu)*(1.0+nu**2/q2)
            sigT = F1/((w2-mp2)/8./pi/pi/alpha/0.3894e3)
            sigL = R*sigT
         else
...
c      if(W2.gt.6.0 .and. Q2.gt.8.0) then
c         call ineft(q2,sqrt(w2),w1tmp,w2tmp,1.0)
c         F1tmp=mp*W1tmp
c         F2tmp=nu*W2tmp
