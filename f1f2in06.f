...
C call Bodek fit (INEFT) for large W and large Q2
        IF(wsq.gt.6.0 .and. qsq.gt.6.0) then
C 4 cases
           xtmp=qsq/2.0/pm/nu
           ...
           IF(wsq.gt.10.0 .or. qsq.gt.10.0) then
              F2=f2a
              F1=f1a
           ELSE
C     First, W^2
...
C Then, Q^2
              F2=(10.0-qsq)/4.0*F2 + (qsq-6.0)/4.0* f2a
              F1=(10.0-qsq)/4.0*F1 + (qsq-6.0)/4.0* f1a
           ENDIF


        ELSEIF(wsq.gt.6.0 .and. qsq.le.6.0) then ! just interpolate in W
           xtmp=qsq/2.0/pm/nu
           call ineft(qsq,sqrt(wsq),W1p,W2p,dble(1.0))
           if(Z.lt.0.5) then
              call ineft(qsq,sqrt(wsq),W1D,W2D,dble(2.0))
        ...
        ELSEIF(qsq.gt.6.0 .and. wsq.le.6.0) then ! just interpolate in Q2
           xtmp=qsq/2.0/pm/nu
           call ineft(qsq,sqrt(wsq),W1p,W2p,dble(1.0))
           if(Z.lt.0.5) then
              call ineft(qsq,sqrt(wsq),W1D,W2D,dble(2.0))
